...
StatusPainter::StatusPainter()
{
    ...
    arc_center_y=painting_area_height/2+100;
    arc_diameter=300.0;
...
void StatusPainter::paintEvent(QPaintEvent *event)
{
    ...
    painter.setFont(QFont("Times",8));//字体
    ...
    painter.restore();
    //画下方地磁方向数值
    painter.setFont(QFont("Times",8,QFont::Bold));//字体
    painter.setPen(QPen(Qt::black,1));//画笔
    painter.setBrush(QBrush(Qt::NoBrush));//画刷不填充
    QRectF compass_rect(painting_area_width/2-15, painting_area_height-12,30,10);
